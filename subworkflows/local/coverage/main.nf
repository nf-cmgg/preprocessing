...
workflow COVERAGE {
    ...
    main:
        ...
        MOSDEPTH(ch_reads_index, ch_target_interval.map{ it -> [[:], it]}, ch_fasta)
        ...
        ch_metrics.groupTuple().map { meta, metrics -> [meta, metrics.flatten()] }.dump(tag: "COVERAGE: metrics", {FormattingService.prettyFormat(it)})
