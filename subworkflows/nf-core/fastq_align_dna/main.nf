...
workflow FASTQ_ALIGN_DNA {
    ...
    main:
        ...
        switch (aligner) {
            case 'bowtie2':
                BOWTIE2_ALIGN(ch_reads_index, false, sort) // if aligner is bowtie2
                ch_bam = ch_bam.mix(BOWTIE2_ALIGN.out.aligned)
                ch_versions = ch_versions.mix(BOWTIE2_ALIGN.out.versions)
                break
            case 'bwamem':
                BWAMEM1_MEM  (ch_reads_index, sort)        // If aligner is bwa-mem
                ch_bam = ch_bam.mix(BWAMEM1_MEM.out.bam)
                ch_versions = ch_versions.mix(BWAMEM1_MEM.out.versions)
                break
            case 'bwamem2':
                BWAMEM2_MEM  (ch_reads_index, sort)        // If aligner is bwa-mem2
                ch_bam = ch_bam.mix(BWAMEM2_MEM.out.bam)
                ch_versions = ch_versions.mix(BWAMEM2_MEM.out.versions)
                break
            case 'dragmap':
                DRAGMAP_ALIGN(ch_reads_index, sort)        // If aligner is dragmap
                ...
                break
            case 'snap':
                SNAP_ALIGN   (ch_reads_index)              // If aligner is snap
